...
public:
	Controladora() {
		...
		oCalavera = new Calavera(100*7.5,100*6.3);
	...
	bool Pruebaa() {
		if (oJugador->retornarRectangulo().IntersectsWith(oCalavera->retornarRectangulo())) {
			return true;
		}
	}
	...
	void dibujar(Graphics^ g, Bitmap^ bmpBase, Bitmap^ bmpSolido, Bitmap^ bmpBomba, Bitmap^ bmpExplosion, Bitmap^ bmpDestruible, Bitmap^ bmpJugador, Bitmap^ bmpEnemigo,Bitmap^bmpCala) {
		...
		oCalavera->dibujarCalavera(g, bmpCala, oEscenario->getmatriz());
